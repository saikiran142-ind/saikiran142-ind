# Hi, I'm Saikiran Goud👋
### IT Support Engineer | Troubleshooting Expert | Documentation Specialist

---

## 👨‍💻 Professional Summary
- 🛠️ Currently working in **Technical IT Support**.
- 🚀 Expert in **Desktop Support**, **OS Optimization**, and **Hardware Diagnostics**.
- 📧 Specialized in **Microsoft Outlook** and **Office 365** administration.
- 📝 Dedicated to creating high-quality **Knowledge Base (KB)** documentation.

## 🛠️ Technical Skills
- **Operating Systems:** 🪟 Windows 10/11, Windows Server (Basics).
- **Email Support:** 📧 Microsoft Outlook (Configuration & Troubleshooting), O365.
- **Networking:** 🌐 TCP/IP, DNS, DHCP, VPN, Wi-Fi & LAN Troubleshooting.
- **Ticketing Tools:** 🎫 Jira Service Management, Zendesk, ServiceNow.
- **Active Directory:** 👥 User Management, Password Resets, Group Policies.
 ### 🛠️ Infrastructure & Security Skills
- **Firewall Admin:** 🧱 Configuring Rules, NAT, and VPN on 'Fortinet' Firewalls.
- **Surveillance:** 📹 **CCTV (IP & Analog)** Installation, NVR/DVR Configuration, and Maintenance.


## 📂 My Technical Wiki (Troubleshooting Guides)
- [📧 Outlook Support Guide](https://github.com/saikiran142-ind/Outlook-Troubleshooting-Guide.) - Detailed fixes for 0x800... errors.
- [🌐 Network Connectivity Handbook](https://github.com/saikiran142-ind/Network-Troubleshooting.md) - CMD tools and Internet fixes.

## 📊 My GitHub Stats

![GitHub Stats]([https://vercel.app[saikiran142-ind])
---

## 🌐 Connect with Me
- **LinkedIn:** www.linkedin.com/in/saikiran-goud-b2a54123a
- **Email:** saikirangoud142@gmail.com

*"I don't just fix computers; I empower users to be more productive."*


